...
double BspFitting::GetBsplinePoint(double x, double y) {

    if ((x < min_pt_.x || x > max_pt_.x || y < min_pt_.y || y > max_pt_.y)) { // 修改成knot_range
        std::cout << "min_pt_.x: " << min_pt_.x << " " << "max_pt_.x: " << max_pt_.x << std::endl;
        std::cout << "min_pt_.y: " << min_pt_.y << " " << "max_pt_.y: " << max_pt_.y << std::endl;
...
double BspFitting::GetBsplinePointLonLat(double lon, double lat) {
    ...
    double query_p_z = GetBsplinePoint(x_intr, y_intr);

    return query_p_z + HOME_LATITUDE;
}
...
//     std::vector<std::vector<Point>> ctr_ptc(M, std::vector<Point>(N)); // ctr_ptc能不能直接使用成员变量
...
//     std::cout << "control point size: " << ctr_points_.size() << " " << ctr_points_[0].size() << std::endl;
// }
