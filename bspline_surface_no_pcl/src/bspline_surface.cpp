...
void BspSurface::SetData(const PointCloud &input, double perc, int kn) {
  ...
  for (const auto &pt : input) {
    ...
    int x_ind =
        std::floor((pt.position.x() - min_pt.position.x()) / grid_size_);
    int y_ind =
        std::floor((pt.position.y() - min_pt.position.y()) / grid_size_);
  ...
  for (int i = 0; i < ct_x_num_; ++i) {
    for (int j = 0; j < ct_y_num_; ++j) {
      auto &pts = grids[i][j];
      PointT ct_pt;
      ct_pt.position.x() = (i + 0.5) * grid_size_ + min_pt.position.x();
      ct_pt.position.y() = (j + 0.5) * grid_size_ + min_pt.position.y();
      if (!pts.empty()) {
  ...
  for (int i = 0; i < ct_x_num_; ++i) {
    for (int j = 0; j < ct_y_num_; ++j) {
      ...
      if (std::isinf(tgt_pt.position.z())) {
        ...
        float query_pt[dim] = {float(tgt_pt.position.x()),
                                           float(tgt_pt.position.y()), 0.0};
        kd_tree.findNeighbors(resultSet, &query_pt[0],
...
//                 coordiantes double y_em = ct_pts_[i][j].position.y();
//                 ct_pts_[i][j].position.z() = InterpolatePoint(ct_pts_pcl_,
//                 x_em, y_em, kn);
//             }
//         }
//     }
