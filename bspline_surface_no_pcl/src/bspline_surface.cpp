...
BspSurface::BspSurface(int k, double grid_size)
    : ku_(k), kv_(k), grid_size_(grid_size) {
  debug_ = true;
  std::cout << "Cons. BSpline Surface." << std::endl;
}

BspSurface::BspSurface(int ku, int kv, double grid_size)
    : ku_(ku), kv_(kv), grid_size_(grid_size) {
  debug_ = true;
  std::cout << "Cons. BSpline Surface." << std::endl;
}

void BspSurface::SetData(const PointCloud &input, double perc, int kn) {
  ...
  if (debug_) {
    std::cout << "min_x: " << min_pt.position.x() << " "
              << "max_x: " << max_pt.position.x() << std::endl;
    std::cout << "min_y: " << min_pt.position.y() << " "
              << "max_y: " << max_pt.position.y() << std::endl;
  }
...
<<<<<<< HEAD
  ...
  int k_ex = 2;
  ct_x_num_ = std::floor(range_x / grid_size_) + 1 + 2 * k_ex;
  ct_y_num_ = std::floor(range_y / grid_size_) + 1 + 2 * k_ex;
=======
  ct_x_num_ = std::floor(range_x / grid_size_) + 1;
  ct_y_num_ = std::floor(range_y / grid_size_) + 1;
>>>>>>> 31653ac34b6463a623f4cd55eea20bc66dfdd0c8
  ...
  for (const auto &pt : input) {
    int x_ind =
        std::floor((pt.position.x() - min_pt.position.x()) / grid_size_) + k_ex;
    int y_ind =
        std::floor((pt.position.y() - min_pt.position.y()) / grid_size_) + k_ex;

    if (x_ind >= 0 && x_ind < ct_x_num_ && y_ind >= 0 && y_ind < ct_y_num_)
      grids[x_ind][y_ind].push_back(pt);
  }
  ...
  for (int i = 0; i < ct_x_num_; ++i) {
    for (int j = 0; j < ct_y_num_; ++j) {
      ...
      PointT ct_pt;
      ct_pt.position.x() =
          (i + 0.5) * grid_size_ + min_pt.position.x() - k_ex * grid_size_;
      ct_pt.position.y() =
          (j + 0.5) * grid_size_ + min_pt.position.y() - k_ex * grid_size_;
      if (!pts.empty()) {

        std::sort(pts.begin(), pts.end(), [](PointT a, PointT b) {
          return a.position.z() < b.position.z();
        });
        int sel_ind = pts.size() * perc;
        ct_pt.position.z() = pts[sel_ind].position.z();

        // float sum_z = 0.0;
        // for (const auto pt: pts) {
        //   sum_z += pt.position.z();
        // }
        // ct_pt.position.z() = sum_z / pts.size();

        ct_pts_pcl_.push_back(ct_pt);

      } else {
        // std::cout << "No data in this grid!!! Set infinity<double>"
        //           << std::endl;
        ct_pt.position.z() = std::numeric_limits<double>::infinity();
        invalid++;
      }
...
void BspSurface::KnnInterplation(int kn) {

  // step: 提取2d点
  PointCloud ct_pts_pcl_2d;
  for (const auto &pt : ct_pts_pcl_) {
    PointT pt_2d;
    pt_2d.position.x() = pt.position.x();
    pt_2d.position.y() = pt.position.y();
    pt_2d.position.z() = 0.;
    ct_pts_pcl_2d.push_back(pt_2d);
  }

  PointCloudAdapter pcAdapter(ct_pts_pcl_2d);

  // 使用nanoflann库中的KDTree实现最近邻搜索
  typedef nanoflann::KDTreeSingleIndexAdaptor<
      nanoflann::L2_Simple_Adaptor<float, PointCloudAdapter>, PointCloudAdapter,
      3 /* 3D 空间 */
      >
      KDTree;

  // 构建 KD-Tree
  KDTree kdtree(3, pcAdapter,
  ...
  for (int i = 0; i < ct_x_num_; ++i) {
    for (int j = 0; j < ct_y_num_; ++j) {
      ...
      if (std::isinf(tgt_pt.position.z())) {

        // 用于保存查询结果
        std::vector<size_t> ret_index(kn);
        std::vector<float> out_dist_sqr(kn);

        nanoflann::KNNResultSet<float> resultSet(kn);
        resultSet.init(&ret_index[0], &out_dist_sqr[0]);
        
        float query_pt[3] = {float(tgt_pt.position.x()),
                             float(tgt_pt.position.y()), 0.0};

        std::cout << "query_pt[0]: " << query_pt[0] << std::endl;
        std::cout << "query_pt[1]: " << query_pt[1] << std::endl;
        ...
        if (resultSet.size() > 0) {
          ...
          for (size_t i = 0; i < resultSet.size(); ++i) {
            sum_z += ct_pts_pcl_[ret_index[i]].position.z();
          }

          ct_pts_[i][j].position.z() = sum_z / kn;
          std::cout << sum_z / kn << std::endl;
        }
      }
    }
  }
...
//   // step: 对于失效高度进行插值
//   for (int i = 0; i < ct_pts_.size(); ++i) {
//     for (int j = 0; j < ct_pts_[0].size(); ++j) {
//       PointT tgt_pt = ct_pts_[i][j];
//       if (std::isinf(tgt_pt.position.z())) {
//         double x_em = ct_pts_[i][j].position.x(); // convert to real
//         double y_em = ct_pts_[i][j].position.y();
//         ct_pts_[i][j].position.z() =
//             InterpolatePoint(ct_pts_pcl_, x_em, y_em, kn);
//       }
//     }
//   }
// }

// double BspSurface::InterpolatePoint(const PointCloud &points, double x,
//                                     double y, int k) {

//   std::priority_queue<std::pair<double, PointT>> pq;

//   // search the nearest k points
//   for (const auto &pt : points) {
//     double dist = std::sqrt((pt.position.x() - x) * (pt.position.x() - x) +
//                             (pt.position.y() - y) * (pt.position.y() - y));
//     pq.push(std::make_pair(dist, pt)); // bug
//     if (pq.size() > k) {
//       pq.pop();
//     }
//   }
...
PointT BspSurface::SampleXY(double sx, double sy) {
  ...
  if (sx < ct_min_pt.position.x() || sy < ct_min_pt.position.y() ||
      sx > ct_max_pt.position.x() || sy > ct_max_pt.position.y()) {
    std::cout << "Err. invalid sample" << std::endl;
    ret.position.z() = std::numeric_limits<double>::infinity();
    return ret;
  }
  ...
  for (int i = 0; i < pts_knots_u_.size() - 1; i++) {
    if (sx >= pts_knots_u_[i].position.x() &&
        sx < pts_knots_u_[i + 1].position.x()) {
      su_i = i;
    }
  }

  for (int i = 0; i < pts_knots_v_.size() - 1; i++) {
    if (sy >= pts_knots_v_[i].position.y() &&
        sy < pts_knots_v_[i + 1].position.y()) {
      sv_i = i;
    }
  }
  ...
  float u_res =
      (sx - pts_knots_u_[su_i].position.x()) /
      (pts_knots_u_[su_i + 1].position.x() - pts_knots_u_[su_i].position.x()) *
      u_span;
  float v_res =
      (sy - pts_knots_v_[sv_i].position.y()) /
      (pts_knots_v_[sv_i + 1].position.y() - pts_knots_v_[sv_i].position.y()) *
      v_span;

  int count = 1;
  PointT init_pt = SampleUV(u_min + u_res, v_min + v_res);
  if (SquareError(sx, sy, init_pt.position.x(), init_pt.position.y()) <
      tolerance) { // 若loss足够小，直接返回拟合点
    if (debug_) {
      std::cout << "x_fit: " << init_pt.position.x() << std::endl;
      std::cout << "y_fit: " << init_pt.position.y() << std::endl;
    }

    ret.position.z() = init_pt.position.z();
    return ret;
  } else { // 重新初始化u_min, v_min, u_max, v_max
    if (init_pt.position.x() < sx) {
      u_min = u_min + u_res;
    } else {
      u_max = u_min + u_res;
    }

    if (init_pt.position.y() < sy) {
      v_min = v_min + v_res;
    } else {
      v_max = v_min + v_res;
    }
  ...
  while ((u_max - u_min > epsilon) && (v_max - v_min > epsilon)) {
    ...
    x_fit = temp.position.x();
    y_fit = temp.position.y();
    ...
    if (error < tolerance) {
      ...
      ret.position.z() = temp.position.z();
      return ret;
    }
...
PointT BspSurface::Sample(const std::vector<PointT> ct_pts,
  ...
  for (int r = 1; r <= k - 1; ++r) {
    for (int i = 0; i <= k - r - 1; ++i) {
      ...
      temp[i].position.x() = factor * temp[i + 1].position.x() +
                             (1 - factor) * temp[i].position.x();
      temp[i].position.y() = factor * temp[i + 1].position.y() +
                             (1 - factor) * temp[i].position.y();
      temp[i].position.z() = factor * temp[i + 1].position.z() +
                             (1 - factor) * temp[i].position.z();
    }
  }
...
double BspSurface::GetHeight(double x, double y) {
  ...
  if (std::isinf(ret.position.z())) {
    return std::numeric_limits<double>::infinity();
  }
  return ret.position.z();
}

void BspSurface::GetSurface(PointCloud &surface, double step) {
...
<<<<<<< HEAD
PointCloud BspSurface::GetValidCtrlPts() { return ct_pts_pcl_; }

PointCloud BspSurface::GetCtrlPts() {
  PointCloud ret;
  for (const auto &ctr_ptcs : ct_pts_) {
    for (const auto &ctr_ptc : ctr_ptcs) {
      ret.push_back(ctr_ptc);
    }
  }
  return ret;
}
=======
PointCloud BspSurface::GetCtrlPts() { return ct_pts_pcl_; }
>>>>>>> 31653ac34b6463a623f4cd55eea20bc66dfdd0c8
