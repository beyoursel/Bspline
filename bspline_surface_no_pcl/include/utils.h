...
struct PointCloudAdapter {
    ...
    // 构造函数
    PointCloudAdapter(const PointCloud &cloud) : cloud(cloud) {}
    ...
    inline float kdtree_get_pt(const size_t idx, const size_t dim) const {
        if (dim == 0) return cloud[idx].position.x();
        else if (dim == 1) return cloud[idx].position.y();
        else return cloud[idx].position.z();
    }

    // 必须实现的 kd-tree 查询 bbox 功能，但这里我们返回 false（不使用）
    template <class BBOX>
